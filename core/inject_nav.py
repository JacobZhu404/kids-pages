...
(function(){
  ...
  var faces=[["#fde047","M42 74 Q50 82 58 74"],["#fca5a5","M42 76 Q50 70 58 76"],["#86efac","M42 72 Q50 84 58 72"],["#93c5fd","M42 78 Q50 74 58 78"]];
  var fi=0;
  function mp(p,cx,cy,mx,my){var r=eggy.getBoundingClientRect();var dx=mx-(r.left+cx*0.72+36),dy=my-(r.top+cy*0.72+42);var d=Math.sqrt(dx*dx+dy*dy)||1,max=4;p.setAttribute('cx',cx+Math.min(max,dx/d*max));p.setAttribute('cy',cy+Math.min(max,dy/d*max));}
  document.addEventListener('mousemove',function(e){mp(pl,42,52,e.clientX,e.clientY);mp(pr,58,52,e.clientX,e.clientY);});
  document.addEventListener('touchmove',function(e){if(e.touches[0]){mp(pl,42,52,e.touches[0].clientX,e.touches[0].clientY);mp(pr,58,52,e.touches[0].clientX,e.touches[0].clientY);}},{passive:true});
...
<div id="kg-eggy" title="点我玩！">
  <svg width="72" height="80" viewBox="0 0 100 110">
    <ellipse class="kg-egg-body" cx="50" cy="58" rx="42" ry="46" fill="#fde047" stroke="#0ea5e9" stroke-width="4"/>
    <ellipse cx="38" cy="34" rx="12" ry="8" fill="#fef9c3" opacity=".6"/>
    <g id="kg-eggy-eyes">
      <ellipse cx="42" cy="50" rx="13" ry="15" fill="#fff" stroke="#0284c7" stroke-width="2.5"/>
      <ellipse cx="58" cy="50" rx="13" ry="15" fill="#fff" stroke="#0284c7" stroke-width="2.5"/>
      <circle id="kg-pupil-l" cx="42" cy="52" r="6.5" fill="#1e293b"/>
      <circle id="kg-pupil-r" cx="58" cy="52" r="6.5" fill="#1e293b"/>
      <circle cx="44" cy="49" r="2.5" fill="#fff"/>
      <circle cx="60" cy="49" r="2.5" fill="#fff"/>
    </g>
    <circle cx="22" cy="62" r="6" fill="#fb7185" opacity=".55"/>
    <circle cx="78" cy="62" r="6" fill="#fb7185" opacity=".55"/>
    <path id="kg-eggy-mouth" d="M42 74 Q50 82 58 74" stroke="#1e293b" stroke-width="3" fill="none" stroke-linecap="round"/>
    <ellipse cx="8" cy="66" rx="6" ry="9" fill="#fde047" stroke="#0ea5e9" stroke-width="3"/>
    <ellipse cx="92" cy="66" rx="6" ry="9" fill="#fde047" stroke="#0ea5e9" stroke-width="3"/>
  </svg>
</div>
<!-- kg-eggy-end -->"""
